...
void drawBoard(char *board, int amountOfRows, int amountOfColumns)
{
    ...
    for (int row = 0; row < amountOfRows; ++row)
    {
        ...
        {
            cout << board[currentElement++]; //Check the current element before increasing it
...
void placeSymbol(char *board, int amountOfRows, int amountOfColumns, players::Players currentPlayer)
{
    ...
    do
    {
        cout << "Which row would you like to place your marker in? ";
        cin >> row;

        cout << "Which column would you like to place your marker in? ";
        cin >> column;

        // Check to see if the selection is not out of bounds
        if (row < amountOfRows && column < amountOfColumns)
        {
            indexOfArray = getIndexInArray(amountOfColumns, row, column);
...
bool checkRows(char *board, int amountOfRows, int amountOfColumns, players::Players currentPlayer)
{
    for (int row = 0; row < amountOfRows; ++row)
    {
        ...
        for (int column = 0; column < amountOfColumns; ++column)
        {
            int indexOfCurrentElement = getIndexInArray(amountOfColumns, row, column);

            if (board[indexOfCurrentElement] == playerToSymbol(currentPlayer))
                ++amountOfWinningCharacters;
        }
        // We check columns on account a row is located on seperate columns 
        // rows are NOT on rows
...
// Checks rows / columns / and diagonals for winning conditions
// TRUE  = someone has won / tied
// FALSE = no game ending conditions found
...
// Asks players if they wish to play, return TRUE if they do, if not return FALSE
bool requestReplay()
{
...
bool gameMain()
{
    ...
    //std::cout << "Game has ended in a tie\n";
    return true;
}
